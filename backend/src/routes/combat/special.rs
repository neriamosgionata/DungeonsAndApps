...
pub async fn two_weapon_fight(
    ...
    if let Some((w, _)) = &offhand_weapon {
        ...
        if props.to_lowercase().contains("thrown") {
            if let Some(chid) = attacker_snap.character_id {
                let _ = super::actions::decrement_thrown_weapon(&mut *tx, chid, wname).await?;
            }
        }
    }
    ...
    if result.hit {
        let _ = super::actions::sync_combatant_hp_to_sheet(&s.db, body.target_id, result.target_hp_after, result.target_temp_hp_after).await;
    }
