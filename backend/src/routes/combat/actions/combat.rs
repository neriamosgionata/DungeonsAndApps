                    ...
                    if parts.len() == 2 {
                        if let Ok(normal_range) = parts[0].trim().parse::<f32>() {
                            if let Ok(long_range) = parts[1].trim().trim_end_matches("ft").trim().parse::<f32>() {
                                ...
                                if dist_ft > normal_range { dis = true; }
                            }
                        }
                    }
                }
...
pub async fn deal_damage(
    ...
    let mut tx = s.db.begin().await?;
    sqlx::query("update combatants set hp_current = $1, temp_hp = $2 where id = $3")
        .bind(result.hp_after).bind(result.temp_hp_after).bind(id)
        .execute(&mut *tx).await?;
...
pub async fn heal(
    ...
    if let Err(e) = sync_combatant_hp_to_sheet(&s.db, id, result.hp_after, target_snap.temp_hp).await {
        tracing::error!(combatant_id = %id, "sync sheet HP: {e}");
    }
