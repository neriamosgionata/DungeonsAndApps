...
pub fn resolve_skill_check(
    ...
    let skill_prof_for_jack = snap
        .skills
        .get(&skill)
        .or_else(|| snap.skills.get(&skill.replace('_', " ")))
        .and_then(|v| v.as_str());
    let is_proficient_for_jack = matches!(
        skill_prof_for_jack,
        Some("prof") | Some("proficient") | Some("expert")
    );
    ...
    let modv = stats
        ...
        .find(|(s, _)| s == &skill)
        .map(|(_, m)| {
            if !is_proficient_for_jack && stats.jack_of_all_trades {
                *m + (pb / 2)
            } else {
                *m
            }
