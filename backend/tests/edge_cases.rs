...
async fn rate_limit_blocks_excessive_requests() {
    ...
    let (s, _) = json_req(&router, "POST", "/api/v1/auth/register", None, Some(json!({
        "email": email,
    ...
    for i in 0..15 {
        let (s, body) = json_req(&router, "POST", "/api/v1/auth/login", None, Some(json!({
            "email": email,
            "password": "wrong"
        }))).await;
...
async fn rbac_blocks_non_member_campaign_access() {
    ...
    let (_, camp) = json_req(&router, "POST", "/api/v1/campaigns", Some(&master_tok),
        Some(json!({ "name": "RBAC Test" }))).await;
    ...
    let (s, _) = json_req(&router, "GET", &format!("/api/v1/campaigns/{cid}"),
...
async fn rbac_allows_member_read_blocks_write() {
    ...
    let (player_tok, player_body) = register_with(&router, "player@test.com", Some(&master_tok)).await;
    let player_id = player_body["user"]["id"].as_str().unwrap();

    // Create campaign and add player
    let (_, camp) = json_req(&router, "POST", "/api/v1/campaigns", Some(&master_tok),
        Some(json!({ "name": "RBAC Test 2" }))).await;
    let cid = camp["id"].as_str().unwrap();

    json_req(&router, "POST", &format!("/api/v1/campaigns/{cid}/members"),
    ...
    let (s2, _) = json_req(&router, "DELETE", &format!("/api/v1/campaigns/{cid}"),
        Some(&player_tok), None).await;
...
async fn error_not_found_returns_404() {
    ...
    let (s, body) = json_req(&router, "GET", "/api/v1/campaigns/nonexistent-uuid",
        Some(&tok), None).await;
...
async fn error_validation_returns_422() {
    ...
    let (s, body) = json_req(&router, "POST", "/api/v1/campaigns", Some(&tok),
        Some(json!({ "name": "" }))).await;

    // Empty name might be 400 or 422 depending on validation
    assert!(s == 400 || s == 422, "invalid data should return 400/422: {}", body);
}
...
async fn error_unauthorized_returns_401() {
    ...
    // Invalid token
    let (s2, _) = json_req(&router, "GET", "/api/v1/auth/me",
        Some("invalid.token.here"), None).await;
...
async fn attack_with_half_cover_bonus() {
    ...
    assert!(result["hit"].is_boolean(), "attack with cover should return hit result");
}
...
async fn concentration_check_on_damage() {
    ...
    assert!(result["concentration_broken"].is_boolean() || result["concentration_roll"].is_object(),
...
async fn handles_special_characters_in_names() {
    ...
    let (s, result) = json_req(&router, "POST", "/api/v1/campaigns", Some(&tok),
        Some(json!({ "name": "Test: Special & Characters <script>alert('xss')</script>" }))).await;
...
async fn handles_very_long_names() {
    ...
    let (s, result) = json_req(&router, "POST", "/api/v1/campaigns", Some(&tok),
        Some(json!({ "name": long_name }))).await;

    // Should either succeed or fail gracefully with validation error
    assert!(s == 201 || s == 400 || s == 422,
        "long names should be handled: {}", result);
}
...
async fn setup_combat(
    ...
    let (_, camp) = json_req(router, "POST", "/api/v1/campaigns", Some(&master_tok),
        Some(json!({ "name": "Mechanics Test" }))).await;
    ...
    let (_, enc) = json_req(router, "POST", &format!("/api/v1/campaigns/{cid}/encounters"),
        Some(&master_tok), Some(json!({ "name": "Test Battle" }))).await;
...
async fn create_target(
    ...
    let (_, target) = json_req(router, "POST", &format!("/api/v1/encounters/{}/combatants", eid),
        Some(tok), Some(json!({ "ref_type": "npc", "npc_id": npc_id, "display_name": "Target",
                     "initiative": 5, "hp_max": 20, "hp_current": 20, "ac": ac }))).await;
