...
// =====================================================================
// CRITICAL REGRESSION TESTS — Sprint 37 (2026-06-23)
// =====================================================================

/// CRIT-1: publish_persist must update the stored payload to include the seq
/// field. Pre-fix: the JSONB bound to the INSERT had no seq (the seq is
/// generated by the trigger), so replay_events returned events with no seq
/// and clients could not advance their cursor. The fix augments the stored
/// payload via UPDATE after the trigger returns the seq.
#[tokio::test]
async fn crit1_publish_persist_stores_seq_in_payload() {
    let src = std::fs::read_to_string(
        std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("../backend/src/ws.rs"),
    )
    .unwrap();
    // The fix must UPDATE the stored payload to merge in the seq field.
    assert!(
        src.contains("UPDATE ws_events SET payload = payload || jsonb_build_object('seq'"),
        "publish_persist must UPDATE the stored payload to inject seq (CRIT-1 fix)"
    );
    // And it must bind both the seq and the row id.
    assert!(
        src.contains(".bind(seq)") && src.contains(".bind(id)"),
        "UPDATE must bind seq and id (CRIT-1 fix)"
    );
}

/// CRIT-2: ws_events.campaign_id must have an FK to campaigns(id) with
/// ON DELETE CASCADE. Pre-fix: orphan rows accumulated when a campaign
/// was deleted and could never be replayed (the campaign_id no longer
/// resolved). Migration 20260623000002 adds the constraint.
#[tokio::test]
async fn crit2_ws_events_has_campaign_fk() {
    let migration = std::fs::read_to_string(
        std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("../migrations/20260623000002_ws_events_fk_and_per_campaign_seq.sql"),
    )
    .unwrap();
    assert!(
        migration.contains("FOREIGN KEY (campaign_id) REFERENCES campaigns(id) ON DELETE CASCADE"),
        "migration 20260623000002 must add FK with ON DELETE CASCADE (CRIT-2 fix)"
    );
}

/// CRIT-2 (part 2): the seq trigger must compute per-campaign MAX(seq)+1
/// under an advisory lock, not call a global sequence's nextval.
#[tokio::test]
async fn crit2_ws_events_seq_is_per_campaign() {
    let migration = std::fs::read_to_string(
        std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("../migrations/20260623000002_ws_events_fk_and_per_campaign_seq.sql"),
    )
    ...
    assert!(
        ...
        "trigger must use pg_advisory_xact_lock for per-campaign serialization (CRIT-2 fix)"
    );
    assert!(
        migration.contains("MAX(seq), 0) + 1")
            && migration.contains("WHERE campaign_id = NEW.campaign_id"),
        "trigger must compute MAX(seq)+1 scoped to campaign_id (CRIT-2 fix)"
    );
    assert!(
        migration.contains("DROP SEQUENCE IF EXISTS ws_events_seq_per_campaign"),
        "old global sequence must be dropped (CRIT-2 fix)"
    );
}

/// CRIT-3: bulk_add_combatants must not Box::leak. Pre-fix: every added
/// combatant triggered two Box::leak calls (one for title, one for body)
/// to satisfy BulkNotification's `&str` fields. With N added combatants
/// this leaked 2N strings per request, with no upper bound.
#[tokio::test]
async fn crit3_bulk_add_does_not_leak() {
    let bulk_src = std::fs::read_to_string(
        std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("../backend/src/routes/combat/combatants/bulk.rs"),
    )
    .unwrap();
    // Strip line + block comments before searching — we only care about
    // actual call sites, not historical notes.
    let bulk_code: String = bulk_src
        .lines()
        .map(|l| {
        ...
        .join("\n");
    assert!(
        !bulk_code.contains("Box::leak"),
        "bulk_add_combatants must not use Box::leak (CRIT-3 fix)"
    );
    let notif_src = std::fs::read_to_string(
        std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("../backend/src/routes/notifications.rs"),
    )
    .unwrap();
    assert!(
        !notif_src.contains("BulkNotification<'a>"),
        "BulkNotification must be owned (String fields), no lifetime (CRIT-3 fix)"
    );
    assert!(
        notif_src.contains("pub kind: String,") && notif_src.contains("pub title: String,"),
