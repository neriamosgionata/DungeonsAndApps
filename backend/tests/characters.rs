...
async fn create_character_in_own_campaign() {
    ...
    let (s, body) = json_req(&router, "POST", &format!("/api/v1/campaigns/{cid}/characters"),
        Some(&player_tok),
        Some(json!({ "name": "Aela", "race": "Human", "level_total": 3 }))).await;
    assert_eq!(s, 201, "{body}");
    assert_eq!(body["name"], "Aela");
    assert_eq!(body["level_total"], 3);
    assert!(body["id"].is_string());

    let (s2, body2) = json_req(&router, "POST", &format!("/api/v1/campaigns/{cid}/characters"),
        Some(&master_tok),
        Some(json!({ "name": "Npc Hero", "level_total": 5 }))).await;
...
async fn create_character_not_member_403() {
    ...
    let (s, _) = json_req(&router, "POST", &format!("/api/v1/campaigns/{cid}/characters"),
        Some(&outsider_tok),
        Some(json!({ "name": "Ghost" }))).await;
    assert_eq!(s, 403);
}
...
async fn list_characters_player_sees_own_only() {
    ...
    json_req(&router, "POST", &format!("/api/v1/campaigns/{cid}/characters"),
        Some(&player_tok), Some(json!({ "name": "Player PC" }))).await;
    json_req(&router, "POST", &format!("/api/v1/campaigns/{cid}/characters"),
        Some(&master_tok), Some(json!({ "name": "Master NPC" }))).await;

    let (s, list) = json_req(&router, "GET", &format!("/api/v1/campaigns/{cid}/characters"),
        Some(&player_tok), None).await;
    assert_eq!(s, 200);
    assert_eq!(list.as_array().unwrap().len(), 1);
    assert_eq!(list[0]["name"], "Player PC");

    let (s2, master_list) = json_req(&router, "GET", &format!("/api/v1/campaigns/{cid}/characters"),
...
async fn read_character_sheet() {
    ...
    let (_, created) = json_req(&router, "POST", &format!("/api/v1/campaigns/{cid}/characters"),
        Some(&player_tok), Some(json!({ "name": "Bilbo", "level_total": 2 }))).await;
    let cid_char = created["id"].as_str().unwrap();

    let (s, body) = json_req(&router, "GET", &format!("/api/v1/characters/{cid_char}"),
        Some(&player_tok), None).await;
    assert_eq!(s, 200);
...
async fn update_sheet_owner_only() {
    ...
    let (_, c) = json_req(&router, "POST", &format!("/api/v1/campaigns/{cid}/characters"),
        Some(&player_tok), Some(json!({ "name": "Rogue" }))).await;
    let char_id = c["id"].as_str().unwrap();

    let (s, updated) = json_req(&router, "PATCH", &format!("/api/v1/characters/{char_id}"),
        Some(&player_tok),
        Some(json!({ "sheet": { "hp": { "max": 20, "current": 20 } } }))).await;
    assert_eq!(s, 200);
    assert_eq!(updated["sheet"]["hp"]["max"], 20);

    let (s2, _) = json_req(&router, "PATCH", &format!("/api/v1/characters/{char_id}"),
        Some(&master_tok),
        Some(json!({ "sheet": { "hp": { "max": 99 } } }))).await;
    assert_eq!(s2, 403);
}
...
async fn delete_character_owner_only() {
    ...
    let (_, c) = json_req(&router, "POST", &format!("/api/v1/campaigns/{cid}/characters"),
        Some(&player_tok), Some(json!({ "name": "Doomed" }))).await;
    let char_id = c["id"].as_str().unwrap();

    let (s_forbid, _) = json_req(&router, "DELETE", &format!("/api/v1/characters/{char_id}"),
        Some(&master_tok), None).await;
    assert_eq!(s_forbid, 403);

    let (s_ok, _) = json_req(&router, "DELETE", &format!("/api/v1/characters/{char_id}"),
        Some(&player_tok), None).await;
    assert_eq!(s_ok, 204);

    let (s_gone, _) = json_req(&router, "GET", &format!("/api/v1/characters/{char_id}"),
        Some(&player_tok), None).await;
...
async fn short_rest_rolls_hit_dice_and_recovers_hp() {
    ...
    let (s, result) = json_req(&router, "POST", &format!("/api/v1/characters/{char_id}/short-rest"),
        Some(&player_tok), Some(json!({ "hit_dice_spent": 2 }))).await;
...
async fn short_rest_cannot_spend_more_hit_dice_than_available() {
    ...
    let (s, body) = json_req(&router, "POST", &format!("/api/v1/characters/{char_id}/short-rest"),
        Some(&player_tok), Some(json!({ "hit_dice_spent": 3 }))).await;
...
async fn long_rest_restores_hp_and_recovers_half_hit_dice() {
    ...
    let (s, result) = json_req(&router, "POST", &format!("/api/v1/characters/{char_id}/long-rest"),
        Some(&player_tok), None).await;
    assert_eq!(s, 200, "{result}");
...
async fn long_rest_resets_spell_slots() {
    ...
    json_req(&router, "POST", &format!("/api/v1/characters/{char_id}/long-rest"),
        Some(&player_tok), None).await;

    let (_, updated) = json_req(&router, "GET", &format!("/api/v1/characters/{char_id}"),
        Some(&player_tok), None).await;
...
async fn award_xp_master_only() {
    ...
    let (_, c) = json_req(&router, "POST", &format!("/api/v1/campaigns/{cid}/characters"),
        Some(&player_tok), Some(json!({ "name": "Hero", "level_total": 1 }))).await;
    let char_id = c["id"].as_str().unwrap();

    let (s_forbid, _) = json_req(&router, "POST", &format!("/api/v1/campaigns/{cid}/award-xp"),
        Some(&player_tok),
        Some(json!({ "character_ids": [char_id], "xp_each": 300 }))).await;
    assert_eq!(s_forbid, 403);

    let (s, result) = json_req(&router, "POST", &format!("/api/v1/campaigns/{cid}/award-xp"),
        Some(&master_tok),
        Some(json!({ "character_ids": [char_id], "xp_each": 300, "reason": "Slew goblins" }))).await;
    assert_eq!(s, 200, "{result}");
...
async fn award_xp_level_up_triggers() {
    ...
    let (_, c) = json_req(&router, "POST", &format!("/api/v1/campaigns/{cid}/characters"),
        Some(&player_tok),
        Some(json!({ "name": "Levelup", "level_total": 1, "sheet": { "xp": 250 } }))).await;
    let char_id = c["id"].as_str().unwrap();

    let (s, result) = json_req(&router, "POST", &format!("/api/v1/campaigns/{cid}/award-xp"),
        Some(&master_tok),
        Some(json!({ "character_ids": [char_id], "xp_each": 100 }))).await;
    assert_eq!(s, 200, "{result}");
...
async fn spell_list_crud() {
    ...
    let (_, c) = json_req(&router, "POST", &format!("/api/v1/campaigns/{cid}/characters"),
        Some(&player_tok), Some(json!({ "name": "Spellbook" }))).await;
    let char_id = c["id"].as_str().unwrap();

    let (s_add, _) = json_req(&router, "POST", &format!("/api/v1/characters/{char_id}/spells"),
        Some(&player_tok), Some(json!({ "spell_id": spell_id, "prepared": true }))).await;
    assert_eq!(s_add, 204);

    let (s_list, spells) = json_req(&router, "GET", &format!("/api/v1/characters/{char_id}/spells"),
        Some(&player_tok), None).await;
    ...
    let (s_patch, _) = json_req(&router, "PATCH",
        &format!("/api/v1/characters/{char_id}/spells/{spell_id}"),
        Some(&player_tok), Some(json!({ "prepared": false, "notes": Some("save for emergencies") }))).await;
    assert_eq!(s_patch, 204);

    let (s_del, _) = json_req(&router, "DELETE",
        &format!("/api/v1/characters/{char_id}/spells/{spell_id}"),
        Some(&player_tok), None).await;
    assert_eq!(s_del, 204);

    let (_, after_del) = json_req(&router, "GET", &format!("/api/v1/characters/{char_id}/spells"),
        Some(&player_tok), None).await;
