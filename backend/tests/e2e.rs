...
async fn campaigns_and_characters_roles() {
    ...
    let (s, c) = json_req(
        ...
        "/api/v1/campaigns",
        Some(&master_tok),
        Some(json!({ "name": "Curse of Strahd" })),
    )
    .await;
    assert_eq!(s, 201);
    ...
    let (s6, upd) = json_req(
        ...
        Some(&player_tok),
        Some(
            json!({ "sheet": { "hp": { "max": 28, "current": 28 }, "abilities": { "str": 16 } } }),
        ),
    )
    .await;
    assert_eq!(s6, 200);
...
async fn user_management_master_only() {
    ...
    let (s_old, _) = json_req(
        &router,
        "POST",
        "/api/v1/auth/login",
        None,
        Some(json!({ "email": "pl@um.com", "password": helpers::TEST_PASSWORD })),
    )
    .await;
    assert_eq!(s_old, 401);

    // new password works
    let (s_new, _) = json_req(
        &router,
        "POST",
        "/api/v1/auth/login",
        None,
...
async fn member_management_master_only() {
    ...
    let (s5, _) = json_req(
        &router,
        "DELETE",
        &format!("/api/v1/campaigns/{cid}/members/{mid}"),
        Some(&master_tok),
        None,
    )
    .await;
...
async fn multiple_characters_per_player_allowed() {
    ...
    let (s2, body) = json_req(
        ...
        &format!("/api/v1/campaigns/{cid}/characters"),
        Some(&player_tok),
        Some(json!({ "name": "Heir", "level_total": 1 })),
    )
    .await;
...
async fn dice_clear_master_only() {
    ...
    json_req(
        ...
        &format!("/api/v1/campaigns/{cid}/members"),
        Some(&master_tok),
        Some(json!({ "email": "pl@d.com", "role": "player" })),
    )
    .await;
...
async fn dice_rolls_persist_and_scope() {
    ...
    let (s, body) = json_req(
        ...
        &format!("/api/v1/campaigns/{cid}/dice"),
        Some(&tok),
        Some(json!({ "expression": "1d20+5", "label": "attack" })),
    )
    .await;
    ...
    let (s2, _) = json_req(
        ...
        &format!("/api/v1/campaigns/{cid}/dice"),
        Some(&tok),
        Some(json!({ "expression": "gibberish" })),
    )
    .await;
    assert_eq!(s2, 400);

    // history
    let (s3, hist) = json_req(
        &router,
        "GET",
        &format!("/api/v1/campaigns/{cid}/dice"),
        Some(&tok),
...
async fn combat_full_flow() {
    ...
    let (s2, _) = json_req(
        ...
        Some(&tok),
        Some(
            json!({ "ref_type": "npc", "npc_id": npc_id, "display_name": "Goblin 1",
                     "initiative": 14, "hp_max": 7, "hp_current": 7, "ac": 15 }),
        ),
    )
    .await;
    assert_eq!(s2, 201);
    ...
    // end
    let (_, ended) = json_req(
        &router,
        "POST",
        &format!("/api/v1/encounters/{eid}/end"),
        Some(&tok),
        None,
    )
    .await;
...
async fn combat_battle_map_tokens() {
    ...
    let (s, updated) = json_req(
        &router,
        "PATCH",
        &format!("/api/v1/encounters/{eid}"),
        Some(&master_tok),
        Some(json!({ "map_image": "https://example/map.png", "map_grid_size": 64 })),
    )
    .await;
    assert_eq!(s, 200);
    ...
    let alice_c = arr
        .iter()
        .find(|c| c["character_id"].as_str() == Some(&alice_char))
        .unwrap()["id"]
        .as_str()
        .unwrap()
        .to_string();
    let bob_c = arr
        .iter()
        .find(|c| c["character_id"].as_str() == Some(&bob_char))
        .unwrap()["id"]
        .as_str()
        .unwrap()
        .to_string();
    ...
    let (s1, moved) = json_req(
        &router,
        "POST",
        &format!("/api/v1/combatants/{alice_c}/move"),
        Some(&alice_tok),
        Some(json!({ "x": 12.5, "y": 30.0 })),
    )
    .await;
    ...
    let (s2, _body) = json_req(
        &router,
        "POST",
        &format!("/api/v1/combatants/{bob_c}/move"),
        Some(&alice_tok),
    ...
    let (s3, moved3) = json_req(
        &router,
        "POST",
        &format!("/api/v1/combatants/{bob_c}/move"),
        Some(&master_tok),
        Some(json!({ "x": 77.0, "y": 88.0 })),
    )
    .await;
    assert_eq!(s3, 200);
    ...
    let (_, clamped) = json_req(
        &router,
        "POST",
        &format!("/api/v1/combatants/{alice_c}/move"),
        Some(&alice_tok),
        Some(json!({ "x": 200.0, "y": -5.0 })),
    )
    .await;
    ...
    .await
    .unwrap();
    let (_, npc_c) = json_req(
        &router,
        "POST",
        &format!("/api/v1/encounters/{eid}/combatants"),
        Some(&master_tok),
        Some(json!({ "ref_type": "npc", "npc_id": npc_id, "display_name": "Gob" })),
    )
    .await;
    ...
    let (s4, _) = json_req(
        &router,
        "POST",
        &format!("/api/v1/combatants/{npc_cid}/move"),
        Some(&alice_tok),
    ...
    let (s5, colored) = json_req(
        &router,
        "PATCH",
        &format!("/api/v1/combatants/{npc_cid}"),
        Some(&master_tok),
        Some(json!({ "token_color": "#8b1a1a", "token_on_map": true })),
    )
    .await;
    assert_eq!(s5, 200);
    ...
    let (s_hp, _body) = json_req(
        &router,
        "PATCH",
        &format!("/api/v1/combatants/{alice_c}"),
        Some(&master_tok),
        Some(json!({ "hp_current": 1 })),
    )
    .await;
    assert_eq!(s_hp, 400);
    let (s_ok, _ok) = json_req(
        &router,
        "PATCH",
        &format!("/api/v1/combatants/{alice_c}"),
        Some(&master_tok),
        Some(json!({ "conditions": ["stunned"] })),
    )
    .await;
    assert_eq!(s_ok, 200);
    // NPC HP still editable by master.
    let (s_npc, _npc) = json_req(
        &router,
        "PATCH",
        &format!("/api/v1/combatants/{npc_cid}"),
        Some(&master_tok),
        Some(json!({ "hp_current": 3 })),
    )
    .await;
    assert_eq!(s_npc, 200);
    ...
    let alice_own = alice_arr
        .iter()
        .find(|c| c["id"].as_str() == Some(&alice_c))
        .unwrap();
    let bob_view = alice_arr
        .iter()
        .find(|c| c["id"].as_str() == Some(&bob_c))
        .unwrap();
    let npc_view = alice_arr
        .iter()
        .find(|c| c["id"].as_str() == Some(&npc_cid))
        .unwrap();
    assert!(
        alice_own["hp_max"].as_i64().unwrap() > 0,
        "alice sees own HP"
    );
