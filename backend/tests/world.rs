...
macro_rules! skip_no_db {
    () => {
        match make_app().await { Some(x) => x, None => return }
    };
}
...
async fn recap_visibility_and_rbac() {
    ...
    let (s, sess) = json_req(&router, "POST", &format!("/api/v1/campaigns/{cid}/sessions"), Some(&mtok),
        Some(json!({ "title": "Session 1", "recap": "gm notes", "visibility": "private" }))).await;
    assert_eq!(s, 201);
    let sid = sess["id"].as_str().unwrap();

    // player cannot list it
    let (_, list) = json_req(&router, "GET", &format!("/api/v1/campaigns/{cid}/sessions"), Some(&ptok), None).await;
    assert_eq!(list.as_array().unwrap().len(), 0);

    // master flips to players
    let (_, _) = json_req(&router, "PATCH", &format!("/api/v1/sessions/{sid}"), Some(&mtok),
        Some(json!({ "visibility": "players" }))).await;

    // player now sees it
    let (_, list2) = json_req(&router, "GET", &format!("/api/v1/campaigns/{cid}/sessions"), Some(&ptok), None).await;
    assert_eq!(list2.as_array().unwrap().len(), 1);

    // player cannot delete
    let (s2, _) = json_req(&router, "DELETE", &format!("/api/v1/sessions/{sid}"), Some(&ptok), None).await;
    assert_eq!(s2, 403);
}
...
async fn factions_npcs_visibility() {
    ...
    let (_, f) = json_req(&router, "POST", &format!("/api/v1/campaigns/{cid}/factions"), Some(&mtok),
        Some(json!({ "name": "Harpers", "visibility": "players" }))).await;
    let fid = f["id"].as_str().unwrap();

    let (_, n) = json_req(&router, "POST", &format!("/api/v1/campaigns/{cid}/npcs"), Some(&mtok),
        Some(json!({ "name": "Volo", "faction_id": fid, "visibility": "private" }))).await;
    let nid = n["id"].as_str().unwrap();

    // player sees faction, not npc
    let (_, fl) = json_req(&router, "GET", &format!("/api/v1/campaigns/{cid}/factions"), Some(&ptok), None).await;
    assert_eq!(fl.as_array().unwrap().len(), 1);
    let (_, nl) = json_req(&router, "GET", &format!("/api/v1/campaigns/{cid}/npcs"), Some(&ptok), None).await;
    assert_eq!(nl.as_array().unwrap().len(), 0);
    let (s, _) = json_req(&router, "GET", &format!("/api/v1/npcs/{nid}"), Some(&ptok), None).await;
    assert_eq!(s, 403);

    // master flips npc to players
    json_req(&router, "PATCH", &format!("/api/v1/npcs/{nid}"), Some(&mtok),
        Some(json!({ "visibility": "players" }))).await;
    let (_, nl2) = json_req(&router, "GET", &format!("/api/v1/campaigns/{cid}/npcs"), Some(&ptok), None).await;
...
async fn maps_and_pins() {
    ...
    let (_, m) = json_req(&router, "POST", &format!("/api/v1/campaigns/{cid}/maps"), Some(&mtok),
        Some(json!({ "name": "Faerûn", "visibility": "players" }))).await;
    ...
    let (_, pins) = json_req(&router, "GET", &format!("/api/v1/maps/{mid}/pins"), Some(&ptok), None).await;
    assert_eq!(pins.as_array().unwrap().len(), 1);
    let (s2, _) = json_req(&router, "POST", &format!("/api/v1/maps/{mid}/pins"), Some(&ptok),
        Some(json!({ "label": "X", "kind": "note", "x": 0.0, "y": 0.0 }))).await;
    assert_eq!(s2, 403);
}
...
async fn group_party_loot_quests() {
    ...
    let (_, party) = json_req(&router, "GET", &format!("/api/v1/campaigns/{cid}/party"), Some(&ptok), None).await;
    assert_eq!(party["gp"], 0);

    // player updates coins
    let (s, party2) = json_req(&router, "PATCH", &format!("/api/v1/campaigns/{cid}/party"), Some(&ptok),
        Some(json!({ "gp": 150, "shared_notes": "split fairly" }))).await;
    assert_eq!(s, 200);
    assert_eq!(party2["gp"], 150);

    // loot
    let (s2, _) = json_req(&router, "POST", &format!("/api/v1/campaigns/{cid}/loot"), Some(&ptok),
        Some(json!({ "name": "Sword +1", "quantity": 1, "value_gp": 1000.0 }))).await;
    assert_eq!(s2, 201);

    // quests — master only create
    let (s3, q) = json_req(&router, "POST", &format!("/api/v1/campaigns/{cid}/quests"), Some(&mtok),
        Some(json!({ "title": "Save the town", "visibility": "players" }))).await;
    assert_eq!(s3, 201);
    assert_eq!(q["status"], "active");
    let (s4, _) = json_req(&router, "POST", &format!("/api/v1/campaigns/{cid}/quests"), Some(&ptok),
        Some(json!({ "title": "Nope" }))).await;
    assert_eq!(s4, 403);
}
...
async fn whisper_between_any_two_members() {
    ...
    let (_, camp) = json_req(&router, "POST", "/api/v1/campaigns", Some(&master_tok),
        Some(json!({ "name": "Whispers" }))).await;
    let cid = camp["id"].as_str().unwrap().to_string();
    json_req(&router, "POST", &format!("/api/v1/campaigns/{cid}/members"), Some(&master_tok),
        Some(json!({ "email": "p1@wh.com", "role": "player" }))).await;
    json_req(&router, "POST", &format!("/api/v1/campaigns/{cid}/members"), Some(&master_tok),
        Some(json!({ "email": "p2@wh.com", "role": "player" }))).await;

    // player → master
    let (s1, _) = json_req(&router, "POST", &format!("/api/v1/campaigns/{cid}/messages"),
        Some(&p1_tok), Some(json!({ "scope": "whisper", "recipient_id": m_id, "body": "pssh" }))).await;
    assert_eq!(s1, 201);

    // player → player (master not involved)
    let (s2, _) = json_req(&router, "POST", &format!("/api/v1/campaigns/{cid}/messages"),
        Some(&p1_tok), Some(json!({ "scope": "whisper", "recipient_id": p2_id, "body": "yo" }))).await;
    assert_eq!(s2, 201);

    // master → player
    let (s3, _) = json_req(&router, "POST", &format!("/api/v1/campaigns/{cid}/messages"),
        Some(&master_tok), Some(json!({ "scope": "whisper", "recipient_id": p1_id, "body": "secret quest" }))).await;
    assert_eq!(s3, 201);

    // p2 sees the p1↔p2 whisper thread
    let (_, thread) = json_req(&router, "GET",
...
async fn messages_campaign_and_whispers() {
    ...
    // campaign chat
    let (s, _) = json_req(&router, "POST", &format!("/api/v1/campaigns/{cid}/messages"), Some(&ptok),
        Some(json!({ "scope": "campaign", "body": "hello party" }))).await;
    assert_eq!(s, 201);

    // whisper from player to master
    let (s2, _) = json_req(&router, "POST", &format!("/api/v1/campaigns/{cid}/messages"), Some(&ptok),
        Some(json!({ "scope": "whisper", "recipient_id": mid, "body": "psst" }))).await;
    assert_eq!(s2, 201);

    // whisper needs recipient
    let (s3, _) = json_req(&router, "POST", &format!("/api/v1/campaigns/{cid}/messages"), Some(&ptok),
        Some(json!({ "scope": "whisper", "body": "no recipient" }))).await;
    assert_eq!(s3, 400);

    // campaign chat list
    let (_, chat) = json_req(&router, "GET", &format!("/api/v1/campaigns/{cid}/messages"), Some(&mtok), None).await;
    assert_eq!(chat.as_array().unwrap().len(), 1);

    // whispers list involving master
    let (_, wh) = json_req(&router, "GET", &format!("/api/v1/campaigns/{cid}/messages?whispers=true&with_user={pid}"),
