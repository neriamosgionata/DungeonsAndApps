...
async fn member_can_send_campaign_message() {
    ...
    let (s, msg) = json_req(
        ...
        Some(&player_tok),
        Some(json!({ "scope": "campaign", "body": "Hello world" })),
    )
    .await;
    assert_eq!(s, 201, "{msg}");
...
async fn non_member_cannot_send_message() {
    ...
    let (s, _) = json_req(
        ...
        &format!("/api/v1/campaigns/{cid}/messages"),
        Some(&outsider_tok),
        Some(json!({ "scope": "campaign", "body": "Hacking in" })),
    )
    .await;
    assert_eq!(s, 403);
}
...
async fn whisper_sent_and_visible_to_both_parties() {
    ...
    let (s, msg) = json_req(
        ...
        &format!("/api/v1/campaigns/{cid}/messages"),
        Some(&player_tok),
        Some(json!({ "scope": "whisper", "recipient_id": master_id, "body": "secret" })),
    )
    .await;
    assert_eq!(s, 201, "{msg}");
...
async fn list_campaign_messages_excludes_whispers() {
    ...
    json_req(
        ...
        &format!("/api/v1/campaigns/{cid}/messages"),
        Some(&player_tok),
        Some(json!({ "scope": "campaign", "body": "public" })),
    )
    .await;
    json_req(
        &router,
        ...
        &format!("/api/v1/campaigns/{cid}/messages"),
        Some(&player_tok),
        Some(json!({ "scope": "whisper", "recipient_id": master_id, "body": "private" })),
    )
    .await;
