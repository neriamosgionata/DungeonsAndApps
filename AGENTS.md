...
**Rage:** `class_feature "rage"` inserts a `combatant_effect` with `damage_resistance: [bludgeoning,piercing,slashing]`, `damage_bonus: +2/+3/+4` (by Barbarian level), `str_check_advantage` + `str_save_advantage` (PHB: STR checks/saves, NOT attack rolls — fixed 2026-08-05). Damage bonus applies to melee STR attacks only. Rage ends the moment the barbarian is KO'd (all damage paths deactivate the effect + drop the condition) and consumes per-rest `sheet.resources` "Rage Uses".
...
*Last updated: 2026-08-04 (Sprint 39: character automation audit — 5 CRIT + 2 MED + 7 HIGH + round-3 regression pass + round-4 MEDIUM fixes (long-rest HD half-of-total, slot-level preservation, persisted `_race_seed` race cleanup, merged auto-seed patches, class-removal save/resource/pool revoke, crit_range no-clobber, initiative override semantics) + round-5 LOW fixes (manual AC override, no-armor shield, medium dex override, monk/natural armor) + round-6 sweep (28 fixes: new-char full HP + `_hp_seeded`, `_race_seed` array deep-compare, `_crit_auto` Champion progression, racial spell-level map, subclass seeding normalization/level-gate/source-tags, thrown-weapon STR, defense-style AC parity, engine reads `weapon.attack_bonus`/`save_bonuses`/casting override, per-pool hit-die rolls, exhaustion L1=check-dis/L3=save-dis, short-rest dead guard + heal floor, long-rest temp clear, ac_base keeps shield, multiclass XP leveling, resource clamp-down, save-toggle persistence, resources.ts table fixes, JoAT passive/initiative, composite-race matching, misc clamps) + M21 aura radius (`routes/combat/aura.rs`, all 4 save paths via `SaveReq.aura_bonus`, hostile-faction excluded) + pact magic dedicated pool (`sheet.pact_slots`, cast spends pact-first, short/long rest refill) + round-7 A-series combat mechanics (Extra Attack enforcement via `attacks_made_this_turn` + `extra_attack_count`, 6 Battle Master maneuvers + `consume_superiority_die` helper, Destroy Undead CR thresholds, Rage ends on unconscious, Countercharm, Deflect Missiles reaction, Shield Master DEX +2, Blind Fighting blindsight, GWM bonus attack via `gwm_bonus_attack_available` + `bonus_action_attack`, ritual rejected mid-combat, `POST /combatants/{id}/fall`, auto Sneak Attack) + round-8 (material components via `sheet.spell_focus` + `components_bypass`, Song of Rest in short rest, Precision Attack on the attack roll + Parry/Interception reactions, Magical Secrets pool `sheet.magical_secrets`, Superior Technique die grant) + round-9 (mounted combat via `combatants.mounted_on` + mount/dismount endpoints + rider-mount move sync + auto-dismount on mount death + mounted advantage by creature size, rally/goading maneuvers, Protection reaction reroll vs pending hit, pending hits carry natural_roll/bonus) + app-level round 1 (user profile avatar upload + nav, `campaigns.settings` house rules jsonb master-only, NPC duplicate endpoint) + app-level round 2 (homebrew spells via `campaign_spells` table + merged list + cast fallback, campaign archive/restore via `archived_at`, bulk invite endpoint) + app-level round 3 (news search, character JSON export/import, bulk delete NPCs/lore/news) + app-level round 4 (SRD equipment catalog picker — search + category tabs for armor/weapon/shield/gear/ammo/packs, in-game calendar via `campaign_calendar` + advance endpoint + FE page) + app-level round 5 (campaign export/import `GET /campaigns/{id}/export` + `POST /campaigns/import`, session attendance via `session_attendance` + FE picker; loading states verified present everywhere — audit item closed) + app-level round 6 (tags via `tags`+`taggings` + NPC chips/filter, bulk level `POST /campaigns/{id}/characters/bulk-level`, calendar weather) + app-level round 7 (player journal via `journal_entries` + FE page, calendar moon phases + holidays) + app-level round 8 (encounter templates `encounter_templates` + spawn-from-template + FE, session↔calendar date `campaign_sessions.calendar_date`) + app-level round 9 (shops/merchants via `shops`+`shop_items` + buy/sell coin economy + FE page) + app-level round 10 (tags everywhere — NPCs/lore/news/quests/maps via reusable `ResourceTagChips` + `TagFilterBar` components, master toggle + read-only badges + per-page filters) + app-level round 11 (calendar polish: session pinning via `calendar_date`, holiday removal, next-holiday countdown, custom-day jump); remaining app-level backlog tracked in `MISSING_FEATURES_AUDIT.md` §10; see `CHARACTER_AUTOMATION_AUDIT.md`; combat system re-audited + fully fixed 2026-08-05 (sprints A–H: 2 CRIT + 24 HIGH + 35 MED + 27 LOW all closed, 758 tests / 0 fail, see `COMBAT_AUDIT.md`)).*
