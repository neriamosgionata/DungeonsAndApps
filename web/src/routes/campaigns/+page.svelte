...
>
  <a
    href="/campaigns"
    class="inline-flex items-center gap-2 font-display tracking-widest text-violet-400"
  >
    <span class="app-mark">
      <svg
        viewBox="0 0 32 32"
      ...
      >
        ...
        />
        <path
          d="M7 20 q2-3 5-3 q1-3 3-3 l2-2 q2-2 5-1 q3 1 4 4 l-1 2 l2 2 v2 l-3 2 l-1-1 l-1 2 l-5 0 l-2-2 l-2 2 l-5-2 l-1 0 z"
          fill="#2c1810"
        />
      </svg>
    </span>
  ...
  <div class="header-controls" class:menu-open={menuOpen}>
    ...
    <button
      class="inline-flex items-center gap-1.5 rounded-md bg-neutral-900/80 border border-neutral-700 px-3 py-1.5 text-sm text-neutral-300 hover:text-white hover:border-neutral-500 backdrop-blur-sm transition-colors"
      onclick={logout}
    >
...
<section class="page-panel">
  <SectionHeader
    title={$_("campaigns.title")}
    subtitle={$_("campaigns.subtitle")}
  >
    {#snippet icon()}<Swords size={18} />{/snippet}
    {#snippet actions()}
      {#if auth.authenticated}
        <CollapsibleAdd
          label={$_("campaigns.new")}
          title={$_("campaigns.new")}
          alignEnd={false}
        >
          {#snippet children({ close })}
            ...
            >
              <ImageUpload
                ...
                kind="campaign"
                size={72}
                label={$_("campaigns.icon")}
              />
              <input
                required
                placeholder={$_("campaigns.new_name")}
                bind:value={name}
                class="w-full rounded-md bg-neutral-900 border border-neutral-700 px-3 py-2"
              />
              <textarea
                placeholder={$_("campaigns.new_desc")}
                bind:value={description}
                rows="5"
                class="w-full rounded-md bg-neutral-900 border border-neutral-700 px-3 py-2 resize-y min-h-32"
              ></textarea>
              <div class="flex justify-end">
                <button class="rounded-md bg-violet-600 px-6 py-2 text-white"
                  >{$_("campaigns.create")}</button
                >
              </div>
            </form>
          {/snippet}
  ...
  {#if items.filter((c) => {
    ...
    <EmptyState
      title={$_("campaigns.empty")}
      hint={$_("campaigns.hint_master")}
    />
  {:else}
