<script lang="ts">
  ...
  import { Compass, MapPin, Users as UsersIcon, X, Upload, Image as ImageIcon } from '@lucide/svelte';
  ...
  // Auto-create or update the single world map on image upload.
  async function setWorldMapImage(url: string | null) {
    try {
      if (maps.length === 0) {
        const m = await Maps.create(cid, { name: 'World', image_key: url, visibility: 'players' });
        selected = m.id as string;
      } else {
        await Maps.update(maps[0].id as string, { image_key: url });
      }
      await load();
    } catch (e) { error = (e as Error).message; }
  }

  const currentMap = $derived(maps.find((m) => m.id === selected) ?? maps[0]);
...
<style>
  ...
  .hdr-icon, .hdr-right { display: flex; justify-content: center; width: 2.5rem; }
  ...
  .corner.tr { top: 0.3rem; right: 0.4rem; transform: rotate(0); }
  ...
  .compass-star {
    ...
    background: radial-gradient(circle at 50% 50%, rgba(244,228,193,0.85) 0%, rgba(244,228,193,0.6) 70%, rgba(244,228,193,0.25) 100%);
  ...
  .compass-star::after { transform: translate(-50%, -50%) rotate(90deg); height: 80%; }
  .cn {
    font-family: 'IM Fell English SC', serif;
    font-weight: 900;
    font-size: 0.85rem;
  ...
  .modal-close {
    ...
    display: grid; place-items: center;
    border-radius: 9999px;
    background: #3a2313; color: #c9a84c;
  ...
  .modal-foot {
    ...
    padding: 0.75rem 1rem;
    border-top: 1px dashed rgba(139,105,20,0.45);
  }
  .btn-cancel {
