...
{#if auth.authenticated}
  <div class="fixed top-0 right-0 z-50 p-3 flex gap-2">
    {#if auth.isAdmin}
      <a href="/admin"
        class="inline-flex items-center gap-1.5 rounded-md bg-neutral-900/80 border border-amber-800/60 px-3 py-1.5 text-xs text-amber-500 hover:text-amber-300 hover:border-amber-600 backdrop-blur-sm transition-colors"
        title="admin">
        <ShieldCheck size={13} />
